...
class InferenceTest < Minitest::Test
  ...
  def test_config_name_wins
    File.write(File.join(@dir, "ask-local.json"), '{"name": "custom"}')
    name, source = Ask::Local::Inference.infer(@dir)
    assert_equal "custom", name
    assert_equal "ask-local.json", source
  end
  ...
  def test_directory_basename_fallback
    ...
    name, source = Ask::Local::Inference.infer(nested)
